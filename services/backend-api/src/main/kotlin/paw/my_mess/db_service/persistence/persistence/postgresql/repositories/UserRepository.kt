...
class UserRepository: GenericRepository<User>(), IUserRepository<User> {
    ...
    override fun getAll(): List<User> {
        return _jdbcTemplate.query("SELECT * FROM users", _rowMapper)
    }

    override fun add(item: User): String? {
        _jdbcTemplate.update("""INSERT INTO users (username, passwordHash, email, avatarPath) VALUES (?, ?, ?, ?)""", item.username, item.passwordHash, item.email, item.avatarPath)
    ...
    override fun get(id: String): User? {
        val userList = _jdbcTemplate.query("SELECT * FROM users WHERE uid = ? ", _rowMapper, id.toLong())
        return if (userList.isEmpty()) null else userList.get(0)
    }

    // daca jdbc update returneaza 0 then INSUCCESS OPERATION
    override fun delete(id: String): Boolean {
        return _jdbcTemplate.update("DELETE FROM users WHERE uid=?", id.toLong()) != 0
    }

    override fun update(id: String, item: User): Boolean {
        val code = _jdbcTemplate.update("""UPDATE users SET username=?, passwordHash=?, email=?, avatarPath=? WHERE uid=?""", item.username, item.passwordHash, item.email, item.avatarPath, id.toLong())
        return code != 0
    }
